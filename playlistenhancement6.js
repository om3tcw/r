...
({
    ...
    applyThumbnail: function(target, url) {
        target.popover({
            ...
            trigger: "hover", // Use hover for automatic display
            content: '<img src="__url" class="__class">'.replace(/__class/, this.klass).replace(/__url/, url)
        });

        // Ensure the popover stays open when hovering over child elements
        target.on('mouseenter', function() {
            $(this).popover('show'); // Show popover when entering target
        });

        target.on('mouseleave', function() {
            $(this).popover('hide'); // Hide popover when leaving target
        });

        // Keep popover visible when hovering over child elements
        target.find('.btn-group, .qe_title').on('mouseenter', function() {
            target.popover('show'); // Keep popover visible
        });

        target.find('.btn-group, .qe_title').on('mouseleave', function() {
            target.popover('hide'); // Hide popover when leaving child elements
        });
    },
